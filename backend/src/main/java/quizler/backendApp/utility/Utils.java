...
public class Utils {
    ...
    public static Document quizToDocument(String userId, JsonObject quizinfo, JsonArray qnsJson) {
        Document quizDoc = Document.parse(quizinfo.toString());

        List<Document> dataFormatted = new LinkedList<>();
        for (JsonValue jsonVal : qnsJson) {
            Document qnsDoc = Document.parse(jsonVal.toString());
            dataFormatted.add(qnsDoc);
        }
    ...
    public static Document topicBasedQuizToDocument(String userId, JsonObject quizinfo, JsonArray qnsJson) {
        Document quizDoc = Document.parse(quizinfo.toString());
    ...
    public static Document quizAttemptToDocument(JsonObject quizAttempt) {
        ...
        // List<Document> answers = new LinkedList<>();
        // for (JsonValue jsonVal : answersArray) {
        //     Document answerDoc = Document.parse(jsonVal.toString());
        //     answers.add(answerDoc);
        // }
        System.out.printf("FORMAT?: %s\n\n", Document.parse(quizAttempt.toString()));
        return Document.parse(quizAttempt.toString());
    }

    // topic based quiz attempt to document
    public static Document topicBasedQuizAttemptToDocument(JsonObject quizAttempt) {
        // JsonArray answersArray = quizAttempt.getJsonArray("answers");
        // List<Document> answers = new LinkedList<>();
        // for (JsonValue jsonVal : answersArray) {
        //     Document answerDoc = Document.parse(jsonVal.toString());
    ...
    public static String quizToJson(Document quizDoc) {
        return quizDoc.toJson();
    }

    // topic based quiz to json
    public static JsonObject topicBasedQuizToJson(Document quizDoc) {
        String userId = quizDoc.getString("userId");

        String quizId = quizDoc.getObjectId("_id").toString();
        String quizTitle = quizDoc.getString("quizTitle");
        String difficulty = quizDoc.getString("difficulty");
        String topic = quizDoc.getString("topic");
        String type = quizDoc.getString("type");
        String questionType = quizDoc.getString("questionType");
        String language = quizDoc.getString("language");
        List<Document> dataFormatted = quizDoc.getList("data", Document.class);
    ...
    public static String quizAttemptToJson(Document quizAttempt) {
        return quizAttempt.toJson();
    }
    ...
    public static String s3dataToJson(Document s3data) {
        return s3data.toJson();
    }
}
